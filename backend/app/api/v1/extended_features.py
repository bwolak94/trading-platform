"""Extended Features API — additional endpoints for trading analysis (features 26–35).

Renamed from features2.py to follow consistent naming conventions.
The URL prefix /features2 is preserved for backwards compatibility.
"""
...
router = APIRouter(prefix="/features2", tags=["extended-features"])
...
async def get_smart_money_flow(symbol: str) -> dict[str, Any]:
    ...
    try:
        ...
        df = await fetcher.fetch_klines(
            symbol.upper().replace("/", ""), "1h", limit=168
        )
...
async def get_signal_invalidation(
    ...
    """Alias for /invalidation-report/{symbol}."""
